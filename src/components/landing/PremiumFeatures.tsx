...
export function PremiumFeatures() {
  ...
  return <section className="relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        <img src={bedroom} alt="" className="w-full h-full object-cover" />
        ...
        <div className="container-wide">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <div>
              ...
              <h2 className="text-section-title text-white mb-6 rounded-none shadow-none opacity-70">
                {t('premium.title')}
              </h2>

              <p className="text-white/70 text-lg mb-10">
                {language === 'ru' ? 'Каждая вилла оснащена передовыми технологиями и материалами премиум-класса для максимального комфорта.' : language === 'en' ? 'Each villa is equipped with cutting-edge technology and premium materials for maximum comfort.' : 'Her villa, maksimum konfor için son teknoloji ve premium malzemelerle donatılmıştır.'}
              </p>

              <div className="grid sm:grid-cols-2 gap-4">
                {features.map((feature, index) => {
                const Icon = feature.icon;
                return <div key={feature.key} className="group relative p-6 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:bg-white/10 hover:border-primary/30 transition-all duration-300">
